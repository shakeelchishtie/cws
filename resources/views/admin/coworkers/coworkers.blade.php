@extends('layouts.app', ['activePage' => 'coworkers'])
...
<div class="container-fluid mt-5 p-5">
    ...
    @if (Session::has('errors'))
        ...
        @endif
        @if (old('old_value') == "update_coworker")
            <script type="text/javascript">
                window.onload = () =>
                {
        ...
        <div class="table-responsive pt-3">
            <table class="table table-flush" id="datatable-basic">
                <thead class="thead-light">
                    <tr>
                        <th>{{__('#')}}</th>
                        <th>{{__('coworker image')}}</th>
                        <th>{{__('Name')}}</th>
                        <th>{{__('email')}}</th>
                        <th>{{__('enable')}}</th>
                        @if(Gate::check('coworker_edit') || Gate::check('coworker_delete') || Gate::check('coworker_show'))
                            <th>{{__('Action')}}</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><img src="{{ url('images/upload/'.$coworker->image) }}" width="50" height="50" class="rounded-circle" alt=""></td>
                        <td>{{ $coworker->name }}</td>
                        <td>{{ $coworker->email }}</td>

                        <td>
                            <label class="custom-toggle">
                                <input type="checkbox" name="status" {{ $coworker->status == 1 ? 'checked' : '' }}
                                    onclick="change_status('admin/coworkers',{{ $coworker->id }})">
                        ...
                        @if(Gate::check('coworker_edit') || Gate::check('coworker_delete') || Gate::check('coworker_show'))
                            <td class="table-actions">
                                ...
                                <a href="#" class="table-action ml-2" data-toggle="modal" data-target="#edit_model"
                                    onclick="coworker_edit({{$coworker->id}})">
                                ...
                                <a href="#" class="table-action ml-2 table-action-delete"
                                    onclick="deleteData('admin/coworkers',{{ $coworker->id }})">
                                ...
                                <a href="#" class="table-action ml-2" data-toggle="modal" data-target="#show_model"
                                    onclick="coworker_show({{$coworker->id}})">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control @error('name') is-invalid @enderror" name="name" type="text"
                                placeholder="{{__('Co-worker name')}}" value="{{ old('name') }}" style="text-transform: none">
                            @error('name')
            ...
            <div class="modal-content">
                <form class="edit_coworker" method="post" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label class="form-control-label">{{__('Co-worker name')}}</label>
                            <input class="form-control @error('name') is-invalid @enderror" name="name" type="text" placeholder="{{__('Co-worker name')}}" id="name" style="text-transform: none">
