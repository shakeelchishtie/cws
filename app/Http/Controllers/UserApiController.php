...
class UserApiController extends Controller
{
    ...
    {
        $data = Notification::where([['user_id', auth()->user()->id],['user_type','user']])->orderBy('id', 'DESC')->get();
    ...
    public function apiAllAppointment()
    {
        ...
        foreach ($data as $value) {
            $value->isReview = Review::where('order_id',$value->id)->exists();
        }
        return response(['success' => true, 'data' => $data]);
    }
    ...
    public function apiShowAppointment($id)
    {
        ...
        $data->isReview = Review::where([['order_id',$data->id],['user_id',auth()->user()->id]])->exists();
        if ($data->isReview == true) {
            $data->review = Review::where('order_id',$data->id)->first(['comment','rate'])->makeHidden(['user']);
        }
        return response(['success' => true, 'data' => $data]);
    }
    ...
    public function apiBookAppoinment(Request $request)
    {
        ...
        $Wapprove_content = NotificationTemplate::where('title', 'worker appointment book')->first();
        $Wnotification_content = $Wapprove_content->notification_content;
        ...
        $Witem = array();
        $Witem['user_id'] = auth()->user()->id;
        $Witem['order_id'] = $appointment->id;;
        $Witem['title'] = 'appointment booked';
        $Witem['message'] = $message1;
        $Witem['user_type'] = 'driver';
        Notification::create($Witem);
        if ($appoint->payment_type == 'FLUTTERWAVE')
            return response(['success' => true , 'data' => $appointment->appointment_id , 'url' => url('flutterPayment/'.$appoint->id)]);
        return response(['success' => true, 'data' => $appointment->appointment_id]);
    }

    // Flutterwave Payment
    public function apiFlutterWavePayment($appointment_id)
    {
        $order = Appointment::find($appointment_id);
        return view('flutterwave.flutterPayment', compact('order'));
    }
    ...
    public function apiPaystackPayment()
    {
        return view('paystack.paystackPayment');
    }
    ...
    public function redirectToGateway()
    {
        ...
        $callback_url = url('transctionPaystack');
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            "Authorization: Bearer sk_test_1b647b96eec331f4c65d4d2f6dc626777f27a649",
