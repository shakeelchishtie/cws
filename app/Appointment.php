...
class Appointment extends Model
{
    ...
    {
        return User::where('id',$this->user_id)->first(['name','image','email','phone']);
    }
    ...
    public function getServiceAttribute()
    {
        ...
        foreach ($serviceIds as $id)
        {
            if(Service::where('id',$id)->first())
                array_push($service, Service::where('id',$id)->first(['id','service_name','duration','description','price','image'])->makeHidden(['category']));
        }
        return $service;
